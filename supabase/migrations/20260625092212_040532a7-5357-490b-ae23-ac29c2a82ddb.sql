...
BEGIN
  ...
  PERFORM set_config('app.claim_push_endpoint', p_endpoint, true);
...
DROP POLICY IF EXISTS "Users can claim push subs - update" ON public.push_subscriptions;
DROP POLICY IF EXISTS "Users manage own push subs - update" ON public.push_subscriptions;
CREATE POLICY "Users manage own or claimed push subs - update"
ON public.push_subscriptions
FOR UPDATE
TO authenticated
USING (
  user_id = auth.uid()
  OR endpoint = current_setting('app.claim_push_endpoint', true)
)
